import { AuthRepository } from "../repository/authSitterRepository.js";
...
export class AuthService {
  ...
  signUpUser = async (
    ...
    // email을 포함하는 객체찾기
    const existUser = await this.authRepository.findUserByEmail(sitterEmail);
    if (existUser) {
  ...
  // 로그인
  signInUser = async (sitterEmail, password) => {
    // email로 유저가 존재하는지 확인
    const existUser = await this.authRepository.findUserByEmail(sitterEmail);
    if (!existUser) {
